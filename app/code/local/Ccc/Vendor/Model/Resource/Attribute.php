...
class Ccc_Vendor_Model_Resource_Attribute extends Mage_Eav_Model_Resource_Entity_Attribute
{
	...
	public function _clearUselessAttributeValues(Mage_Core_Model_Abstract $object)
	{
		$data = $object->getOrigData();
		if ($object->isScopeGlobal() && isset($origData['is_global']) && Ccc_Vendor_Model_Resource_Eav_Attribute::SCOPE_GLOBAL != $origData['is_global']) {
			...
			if (!empty($attributeStoreIds)) {
				...
				$this->_getWriteAdapter()->delete($object->getBackendTable(), $delConditions);
			}
		}
		return $this;
	}
	...
	public function deleteEntity(Mage_Core_Model_Abstract $object)
    {
        ...
        if ($result) {
            $attribute = Mage::getSingleton('eav/config')
                ->getAttribute(Ccc_Vendor_Model_Resource_Vendor::ENTITY, $result['attribute_id']);
